...
bool AdsServiceImplIOS::ShouldClearAdsData(const std::string& path) const {
  // Only clear ads data when Sponsored Ads is disabled for users who have
  // not joined Brave Rewards, or when the user disables Brave Rewards.
...
void AdsServiceImplIOS::OnAdsPrefChanged(const std::string& path) {
  ...
  base::SequencedTaskRunner::GetCurrentDefault()->PostTask(
      FROM_HERE,
      base::BindOnce(&AdsServiceImplIOS::MaybeClearDataForDisabledSponsoredAds,
                     weak_ptr_factory_.GetWeakPtr(), path));
}

void AdsServiceImplIOS::MaybeClearDataForDisabledSponsoredAds(
